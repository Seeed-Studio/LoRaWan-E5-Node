...
uint32_t FLASH_If_Write(uint32_t FlashAddress, uint32_t* Data ,uint32_t DataLength)
{
  ...
  for (i = 0; (i < DataLength) && (FlashAddress <= (USER_FLASH_END_ADDRESS-8)); i++)
  {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
       be done by word */
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, FlashAddress, *((uint64_t *)(Data+2*i))) == HAL_OK)
    {
...
uint32_t FLASH_If_WriteByte(uint32_t FlashAddress, uint8_t* Data ,uint32_t DataLength)
{
  ...
  for (i = 0; (i < DataLength) && (FlashAddress <= USER_FLASH_END_ADDRESS); i++)
  {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
       be done by word */
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FAST, FlashAddress, Data[i]) == HAL_OK)
    {
     /* Check the written value */
